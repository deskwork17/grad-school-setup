...
- **`.claude/commands/send-file.md`** — a `/send-file <path> [email]` command that solves a real, recurring problem: sending a file through the Gmail/Drive MCP tools requires reproducing the whole file as base64 inside the tool call, which gets token-expensive and risks corruption past trivial sizes. This uploads the file straight from disk via `rclone copyto` instead, gets a shareable link via `rclone link`, and either emails just that link or hands it back in chat. Depends on the Drive mirror from section 4 — build that first.
